...
function App() {
  ...
  const VIEW_DESC: Record<ViewMode, { en: string; ru: string; zh: string; title_en: string; title_ru: string; title_zh: string }> = {
    ...
    formatriply: { title_en: 'Form A Triply', title_ru: 'Form A: 3 пути', title_zh: 'Form A三重', en: 'Self-energy Σ = −L = −7 fixed by three independent routes: anchor Fricke-pair (X.423), Grothendieck whole-eigensummand (X.247c.cond), Fricke-pair log-residue (G.10C). X.247c [CONJ HEADLINE] preserved.', ru: 'Собственная энергия Σ = −L = −7 фиксируется тремя независимыми маршрутами: anchor Fricke-pair (X.423), Grothendieck whole-eigensummand (X.247c.cond), Fricke-pair log-residue (G.10C). X.247c [CONJ HEADLINE] сохранён.', zh: '自能 Σ = −L = −7 由三个独立路径确定。X.247c [CONJ HEADLINE] 保留。' },
    fourroutes:  { title_en: 'Four Routes to (2,3)', title_ru: 'Четыре пути к (2,3)', title_zh: '四条路径到(2,3)', en: 'The pair (d₁, d₂) = (2, 3) is the unique solution of four independent equations: Catalan (d₂−d₁=1), NCG (Connes A_F), Ihara (d₁³=N+2), Mihailescu (d₂²−d₁³=1). Not a fit.', ru: 'Пара (d₁, d₂) = (2, 3) — единственное решение четырёх независимых уравнений: Catalan, NCG (Connes A_F), Ihara, Mihailescu. Это не подгонка.', zh: '配对 (d₁, d₂) = (2, 3) 是四个独立方程的唯一解：Catalan、NCG、Ihara、Mihailescu。' },
  };
    ...
    <div className="space-y-4">
      <Card className={`${theme.cardBg} ${theme.border} border`}>
        ...
        <CardContent className="space-y-2">
                  ...
                  >
                    <div className="flex items-center gap-2">
                      <span className="text-[#FFD700] font-mono font-bold">Σ=−L</span>
      ...
      <header className={`${theme.headerBg} border-b ${theme.border} px-4 py-3`}>
        <div className="flex items-center justify-between max-w-[1920px] mx-auto">
          ...
          <div className="flex items-center gap-2 md:gap-3">
            ...
            <a href="https://zenodo.org/records/20257066" target="_blank" rel="noopener noreferrer"
